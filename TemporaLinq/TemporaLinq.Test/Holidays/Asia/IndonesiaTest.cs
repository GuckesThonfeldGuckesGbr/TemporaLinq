...
public class IndonesiaTest
{
    ...
    public void GetHolidays_ContainsFixedHolidays()
    {
        ...
        holidays.Should().Contain(h => h.Date == new DateOnly(2026, 8, 17) && h.Name == IndependenceDay);
        holidays.Should().Contain(h => h.Date == new DateOnly(2026, 12, 25) && h.Name == ChristmasDay);
    }

    [Fact]
    public void GetHolidays_ContainsChristianHolidays()
    {
        var holidays = NationalHolidays.Create().From(new DateOnly(2026, 1, 1)).To(new DateOnly(2026, 12, 31));
        var easter = EasterSundayCalculation.Christian.ForYear(2026);

        holidays.Should().Contain(h => h.Date == easter.AddDays(-2) && h.Name == GoodFriday);
        holidays.Should().Contain(h => h.Date == easter.AddDays(39) && h.Name == AscensionDay);
    }
